...
```text
https://ijudge.it.kmitl.ac.th/submissions/600528/overview
```
...
```text
Input1: รับค่าจำนวนเต็มสองจำนวนคือ S , N
Input2: เป็นแบบ loop ตามจำนวน N

Output: คารบอกเวลาที่หมึกจะมาถึงเป็นจำนวน N

Constraints: ต้องคำนวนหาสูตรระยะห่างระหว่างจุด
```
...
```text
step1: อ่านค่า N แล้วรับ input() แบบ loop ตามจำนวน N
step2: รับค่าตำแหน่ง (x , y) และทำการคำนวนหาว่าเวลาที่หมึกจะมาถึงจุด (x , y) แล้วเก็บคำตอบที่ได้มาไว้ใน list
step3: ปริ้นคำตอบจาก list
```
...
```text
บอกฉันว่าตัวปรับให้เลขทศนิยมขึ้นตลอดคือ math.ceil
```

คุณยังทำอะไรด้วยตนเอง

```text
คำนวณสูตรแล้วเขียนการใช้ loop
```
